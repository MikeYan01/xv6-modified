/*
Part 2:
Do you see a change in the timing? Did you expect that?

After running for lots of times, we found that as the number of threads increases,
the spinlock seems to be more efficient than the mutex.

We thought this is because mutex locks keep putting threads to sleep and waking them
up, which requires lots of CPU instructions and costs much time as the number 
of threads grows. However, spinlocks continuously check the lock until it is obtained, 
each costs CPU time constantly. Thus, it is more time-consuming if we only have few threads,
but as the number of threads grows, it becomes somewhat efficient when compared with mutex. 
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    ...
    /********** Q2 Modify Begin **********/
    pthread_spin_init(&spinlock, 0); // initialize lock
    /********** Q2 Modify End **********/
